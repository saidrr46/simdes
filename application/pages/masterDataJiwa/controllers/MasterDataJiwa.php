...
class MasterDataJiwa extends MX_Controller
{
	...
	public function get_data() {
		$data = $this->MasterDataJiwaModel->get_data_dusun();
		$output = '';
	...
	public function select_data(){
		$data = $this->MasterDataJiwaModel->get_dusun_by_id($this->input->post('id'));
	...
	public function update_data (){
		...
		if($par['action'] === 'edit') {
			$id = $par['id_dukuh'];
			$q = $this->MasterDataJiwaModel->edit_data($id, $data);
			if ($q) {
			...
			if ($data) {
				$q =  $this->MasterDataJiwaModel->add_data($data);
				if ($q) {
					$resp = array('status' => 'sukses');
					echo json_encode($resp);
				} else {
	...
	public function delete_data(){
		$data = $this->MasterDataJiwaModel->delete_data($par = $this->input->post('id'));
		echo json_encode($data);		
	}
}
